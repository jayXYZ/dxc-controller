...
function CardImagePicker() {
    ...
    async function handleSearchClick(item: string) {
        ...
        socket.emit("update_data", {...data, cardimage: printings[ogprinting]?.image_uris?.png ?? ''});
    }
    ...
    function handlePrintSelect(value: string) {
        ...
        socket.emit("update_data", {...data, cardimage: prints[parseInt(value)]?.image_uris?.png ?? ''});
    }
        ...
        <Card className="">
            <CardHeader>
                <LiveSearch 
                    ...
                    onSelect={(item) => handleSearchClick(item)}
                />
            </CardHeader>
            <CardContent>
                <img src={data.cardimage} className="w-full object-contain min-h-0 max-h-[60vh]" />
            </CardContent>
            <CardFooter>
                <Select value={selectedIndex.toString()} onValueChange={handlePrintSelect}>
