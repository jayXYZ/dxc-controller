...
const Deck2 = () => {
  ...
  useEffect(() => {
    const fetchDeckData = async () => {
      ...
      try {
        const parsed = await parseDecklist(data.p2decklist);
  ...
  return (
    <div className="w-[1920px] h-[1080px] bg-black text-white p-4 overflow-hidden">
      <div className="h-full">
        <div className="mx-2">
          <span className="text-[48px] font-bold">{data.p2name}</span>
          <span className="text-[36px] font-thin px-2 text-[#C6AD65]">{data.p2deck}</span>
        </div>
